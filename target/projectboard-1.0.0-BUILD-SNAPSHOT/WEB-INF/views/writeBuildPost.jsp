...
<meta charset="UTF-8">
	...
	<style>
		...
		* {
		...
		#editortitle {
			...
			border-bottom: 1px solid rgb(79 87 112);
		}
	...
	<script>
		let checkSerch = "";
	   	...
	   	$(document).ready(function() {
	   	    renderAll(checkPoint);
	   	});
	   	$(document).ready(function() {
	   		$('#imageBox').on('click', 'img', function () {
	   		    const url = $(this).attr('src');
	   		    console.log('클릭된 이미지 src:', url);
	   		 	const markdown = "![](" + url + ")\n";
	   		    const current = editor.getMarkdown();
	   		    editor.setMarkdown(current + markdown);
	   		});
	   	});
	   	$(function() {
	   	...
	   	function renderAll(checkPoint) {
	   	    let url = "";
	   	    if (checkPoint === 1) url = 'getBuildPostWriteChampionImage';
	   	    else if (checkPoint === 2) url = 'getBuildPostWriteItemImage';
	   	    else if (checkPoint === 3) url = 'getBuildPostWriteRuneImage';
	   	    else if (checkPoint === 4) url = 'getBuildPostWriteSpellImage';
	   	 	$("#imageBox").empty();
	   	    $.ajax({
	   	        url: url,
	   	        method: 'GET',
	   	        dataType: 'json',
	   	     	data: { inputName: checkSerch },
		   	    success: function(data) {
	   	    	data.forEach(item => {
	   	    	    const div = $('<div>').attr('id', item.id).css({ display: 'flex', alignItems: 'center', gap: '8px' });
	   	    	    const img = $('<img>').attr('src', item.url).css({ width: '80px', height: '80px' });
	   	    	    const name = $('<div>').text(item.name);
	...
	<script>
	    const inputId = "${sessionScope.id}";
	</script>
	<script>
	    const ws = new WebSocket("ws://localhost:9090/project/chat");
	...
	<script>
	    function checkSelect() {
	        const select = document.getElementById('selsectId');
	        if (select.value === 'opt0') {
	            alert('챔피언을 선택해주세요');
	            alert("<%= request.getParameter("bbno") %>");
	            return false;
	        }
					...
					<div>
						<div id="content">
							<div class="content_inner">
								<form action="buildPostWrite" method="post" onsubmit="return checkSelect()">
									<div>
										<div id="editortitle">빌드 연구소 게시물 작성 및 수정</div>
										<div id="editorheader">
											<select id="selsectId" name="selectId">
												<option id="opt0" value="opt0">챔피언 선택</option>
												<c:forEach var="champ" items="${SelectList}">
												    <option id="opt${champ.champId}" value="${champ.champId}">${champ.champName}</option>
												</c:forEach>
											</select>
											<input id="inputTitle" type="text" name="title" placeholder="제목" required/></div>
										<div id="editor"></div>
										<div>
											<div></div>										
										</div>
										<textarea name="content" id="contentField" style="display:none;"></textarea>
										<input type="hidden" name="bno" value="${param.bbno != null ? param.bbno : ''}" />
							...
							<div id="itemBoxContainer">
								<div id="searchBoxheader">
									<button id="championBtn" name="championBtn">챔피언</button>
									<button id="itemBtn" name="itemBtn">아이템</button>
									<button id="runeBtn" name="runeBtn">룬</button>
									<button id="spellBtn" name="spellBtn">스펠</button>
									<input type="search" id="searchBox" placeholder="검색어 입력" />
								</div>
								<div id="imageBox">
									<div></div>
									<div></div>
